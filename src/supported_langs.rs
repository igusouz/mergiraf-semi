...
pub static SUPPORTED_LANGUAGES: LazyLock<Vec<LangProfile>> = LazyLock::new(|| {
   ...
   right: (list) @commutative)
        ...
        LangProfile {
            ...
            commutative_parents: vec![
                CommutativeParent::without_delimiters("stylesheet", "\n"),
                CommutativeParent::new("block", "{", "\n", "}")
                    .restricted_to_groups(&[
                        &["declaration"],
                    ]),
                CommutativeParent::new("style_rule", "", "\n\n", "")
                    .restricted_to_groups(&[
                        &["rule_set", "style_rule"],
                    ]),
            ],
            signatures: vec![
                signature("declaration", vec![ vec![Field("property")] ]),
                signature("style_rule", vec![ vec![Field("selector")] ]),
